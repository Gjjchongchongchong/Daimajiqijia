        ...
        <ul class="cart-list" v-for="(cart,index) in cartInfoList"
          ...
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cart.isChecked"
              @click="isCheckedChange(cart,$event)">
          </li>
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)" class="mins"
              @click="skuNumChange('minus',-1,cart)">-</a>
            <input autocomplete="off" type="text" :value="cart.skuNum"
              minnum="1" class="itxt"
              @change="skuNumChange('input',$event.target.value*1,cart)">
            <a href="javascript:void(0)" class="plus"
              @click="skuNumChange('add',1,cart)">+</a>
          </li>
          <li class="cart-list-con6">
            <span class="sum">{{cart.cartPrice*cart.skuNum}}</span>
          </li>
          <li class="cart-list-con7">
            <a class="sindelet" @click="DeleteGoodsList(cart.skuId)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    getData () {
      this.$store.dispatch('reqGetShopCarList')
    },
    // 点击全选
    ...
    skuNumChange: throttle(async function (type, disNum, cart) {
      ...
      try {
        // d代表修改成功
        await this.$store.dispatch('PostShopCar', { skuid: cart.skuId, skuNum: disNum })
        // 再次获取服务器最新的数据进行展示
        this, this.getData()
      } catch (error) {
    ...
    // 删除商品
    async DeleteGoodsList (skuId) {
      // console.log(skuId);
      try {
        await this.$store.dispatch('reqDeleteGoodsList', skuId)
        // console.log(result);
        this.getData()
      } catch (error) {
    ...
    // 变更选中状态
    async isCheckedChange (cart, event) {
      try {
        let isChecked = event.target.checked ? '1' : '0'
        // console.log(isChecked);
        await this.$store.dispatch('reqChangeCheckedStatus', { skuId: cart.skuId, isChecked })
        this.getData()
      } catch (error) {
        alert('error.message')
      }
